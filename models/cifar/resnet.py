...
class Generator(nn.Module):
    def __init__(self, isize, z_dim, nc, ngf, ngpu):
        ...
        self.ngpu = ngpu
        self.ngf = ngf
        self.dense = nn.ConvTranspose2d(self.z_dim, ngf, 4, stride=1, padding=0)
...
class Discriminator(nn.Module):
    def __init__(self, isize, nc, ndf, ngpu, norm_type = 'none', loss_type='wgan'):
        ...
        self.main = nn.Sequential(
                FirstResBlockDiscriminator(nc, ndf, stride=2, myConv2d = self.myConv2d),
                ResBlockDiscriminator(ndf, ndf, stride=2, myConv2d = self.myConv2d, use_BN = use_BN),
                ResBlockDiscriminator(ndf, ndf, myConv2d = self.myConv2d, use_BN = use_BN),
                ResBlockDiscriminator(ndf, ndf, myConv2d = self.myConv2d, use_BN = use_BN),
                nn.ReLU(),
                nn.AvgPool2d(8),
                self.myConv2d(ndf, 1, 1, 1, 0, bias=USE_BIAS)
            )
        if loss_type == "dcgan":
