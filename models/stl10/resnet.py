...
class Generator(nn.Module):
    def __init__(self, isize, z_dim, nc, ngf, ngpu):
        ...
        self.z_dim = z_dim
        self.ngpu = ngpu
        self.dense = nn.ConvTranspose2d(self.z_dim, 512, 6, stride=1, padding=0)
...
class Discriminator(nn.Module):
    def __init__(self, isize, nc, ndf, ngpu, norm_type = 'none', loss_type='wgan'):
        ...
        self.main = nn.Sequential(
                ...
                ResBlockDiscriminator(512, 1024, myConv2d = self.myConv2d, use_BN = use_BN),
                nn.ReLU(),
                nn.AvgPool2d(6),
                self.myConv2d(1024, 1, 1, 1, 0, bias=USE_BIAS)
            )
        if loss_type == "dcgan":
